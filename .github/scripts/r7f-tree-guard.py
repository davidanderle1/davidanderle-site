...
def manifest(root: Path, output: Path, source_mode: bool) -> None:
    rows = scan(root, source_mode)
    result = {
        'root': str(root.resolve()),
        'sourceMode': source_mode,
        'entryCount': len(rows),
        'treeSha256': tree_digest(rows),
        'entries': rows,
    }
...
def compare(first: Path, second: Path, output: Path, source_mode: bool) -> None:
    ...
    changed = sorted(
        path for path in set(first_by_path) & set(second_by_path)
