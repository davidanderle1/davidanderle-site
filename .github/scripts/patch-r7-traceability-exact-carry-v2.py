...
def main() -> None:
    ...
    if 'additionalCarryForwardIds' in text or 'expected_carry_ids==set(cids)' in text:
        raise SystemExit('exact carry-forward repair already partially present')
    ...
    text = replace_once(
        ...
        'self-test exact carry-forward check',
    )
    if "all(r['status'].startswith('DEFERRED_BOUND_')" not in text:
        raise SystemExit('future-programme deferral invariant unexpectedly absent')
