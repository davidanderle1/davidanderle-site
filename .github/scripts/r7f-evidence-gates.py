...
def builder_gate(args: argparse.Namespace) -> dict[str, Any]:
    ...
    record(checks, "gate-ready", (root / "R7E_GATE_DECISION.txt").read_text().strip() == "R7E PORTABLE JSON SCHEMA BUILD EVIDENCE COMPLETE — READY FOR INDEPENDENT R7F")
    ...
    record(
        ...
        args.expected_artifact_name == f"r7e-portable-self-recording-final-v3-evidence-{args.expected_commit}"
        and args.expected_artifact_digest.startswith("sha256:")
        and len(args.expected_artifact_digest) == 71,
    )
