...
data.update({'passed':False,'verificationVersion':'R7F-v6-fingerprint-bound-locked','lockedExitStatus':int(sys.argv[1])})
...
on_exit() {
  local status=$?
  if [[ "$LOCKED_SUCCESS" -ne 1 ]]; then stage_failure "$status"; fi
  exit "$status"
}
trap on_exit EXIT
...
result={'gate':'R7F_V6_IMMUTABLE_FINGERPRINT_BOUND_INPUT_LOCK','passed':all(checks.values()),'checks':checks,'failedChecks':[k for k,v in checks.items() if not v],'lock':lock}
Path('/tmp/r7f-v6-lock-preflight.json').write_text(json.dumps(result,indent=2)+'\n')
print(json.dumps(result,indent=2))
if not result['passed']: raise SystemExit(1)
PY
sha256sum "$LOCK" "$ROOT/.github/scripts/r7f-v6-locked.sh" > /tmp/r7f-v6-lock-infrastructure.sha256
...
checks.update({
 ...
 'locked-fingerprint-inventory':lock.get('builderFingerprintInventorySha256')==(data.get('builderInputLock') or lock).get('builderFingerprintInventorySha256'),
...
data.update({'passed':all(checks.values()),'verificationVersion':'R7F-v6-fingerprint-bound-locked','checks':checks,'failedChecks':[name for name,passed in checks.items() if not passed],'builderInputLock':lock})
p.write_text(json.dumps(data,indent=2)+'\n')
