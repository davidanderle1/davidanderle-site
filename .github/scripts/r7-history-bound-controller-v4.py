...
def evaluate(
    ...
    adapter_checks = {
        ...
        'r7f-v5-schema': r7f.get('schema') == 'R7F_PORTABLE_JSON_SCHEMA_AUTHORITATIVE_TUPLE_V5',
        'r7f-v5-correction-model': r7f.get('correctionModel') == 'EXACT_FOUR_CLASS_PATH_REPAIR',
        'external-v4-schema': external.get('schema') == 'R7_PORTABLE_EXTERNAL_AUDIT_TUPLE_V4',
        'external-v4-correction-model': external.get('correctionModel') == 'EXACT_FOUR_CLASS_PATH_REPAIR',
    }
    if not all(adapter_checks.values()):
        failed = [name for name, passed in adapter_checks.items() if not passed]
        return {
            'schema': 'R7_HISTORY_BOUND_FINAL_CONTROLLER_V4',
            'passed': False,
            ...
            'compatibilityAdapter': {
                ...
                'allowedTransformations': [
                    'R7F tuple schema V5 to the core V4 label only',
                    'External audit tuple schema V4 to the core V3 label only',
                ],
    ...
    result.update({
        ...
        'compatibilityAdapter': {
            'passed': True,
            'allowedTransformations': [
                {'field': 'r7f.schema', 'from': 'R7F_PORTABLE_JSON_SCHEMA_AUTHORITATIVE_TUPLE_V5', 'toCoreLabel': 'R7F_PORTABLE_JSON_SCHEMA_AUTHORITATIVE_TUPLE_V4'},
                {'field': 'external.schema', 'from': 'R7_PORTABLE_EXTERNAL_AUDIT_TUPLE_V4', 'toCoreLabel': 'R7_PORTABLE_EXTERNAL_AUDIT_TUPLE_V3'},
            ],
            'digestOrLinkMutationAllowed': False,
