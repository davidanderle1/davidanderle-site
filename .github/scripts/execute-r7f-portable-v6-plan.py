...
WRANGLER_RELATIVE_BLOCK = '''          test "$(find r7f-evidence/wrangler -type f -print -quit)"
          find r7f-evidence/wrangler -type f -print | sort > r7f-evidence/wrangler-files.txt
'''
WRANGLER_ABSOLUTE_BLOCK = '''          test "$(find "$GITHUB_WORKSPACE/r7f-evidence/wrangler" -type f -print -quit)"
...
def main() -> None:
    if len(sys.argv) != 5:
        raise SystemExit('usage: execute-r7f-portable-v6-plan.py INPUT_YAML CORRECTED_YAML DIFF REPORT')
    base.main()
    report_path = Path(sys.argv[4]).resolve()
    report = json.loads(report_path.read_text(encoding='utf-8'))
    report['schema'] = 'R7F_CORRECTED_PLAN_EXECUTION_V6'
    report['correctionModel'] = 'EXACT_FIVE_CLASS_PATH_REPAIR'
