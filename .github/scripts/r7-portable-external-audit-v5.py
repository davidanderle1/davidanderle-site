...
def semantic(root: Path, status: dict, builder_extract: Path | None) -> dict:
    ...
    expected_builder_counts: dict[str, int] = {}
    ...
    for old, new, label in PATH_CORRECTIONS:
        ...
        require(count > 0, f'builder-bound plan lacks path anchor: {label}')
        expected_final = expected_final.replace(old, new)
        expected_corrections.append({'label': label, 'from': old, 'to': new, 'count': count})

    checks = {
        'status-schema': status.get('schema') == 'R7F_PORTABLE_JSON_SCHEMA_AUTHORITATIVE_TUPLE_V7',
        'status-pass': status.get('passed') is True,
        'status-correction-model': status.get('correctionModel') == 'EXACT_FIVE_CLASS_PATH_REPAIR_R2',
        'status-correction-count': status.get('correctionClassCount') == 5,
        ...
        'builder-plan-byte-exact': builder_plan_path.read_text(encoding='utf-8') == expected_builder,
        ...
        'execution-schema': execution.get('schema') == 'R7F_CORRECTED_PLAN_EXECUTION_V7',
        'execution-pass': execution.get('passed') is True,
        'execution-correction-model': execution.get('correctionModel') == 'EXACT_FIVE_CLASS_PATH_REPAIR_R2',
        'execution-correction-count': execution.get('correctionClassCount') == 5,
    ...
    require(not failed, f'portable external audit v5 failed: {failed}')
...
def selftests(root: Path, status: dict) -> dict:
    cases = [
        ...
        ('correction-target', 'R7F_PLAN_EXECUTION.json', lambda data: data['corrections'][0].__setitem__('to', '/tmp/fake')),
        ('wrangler-anchor', 'R7F_PLAN_EXECUTION.json', lambda data: data['corrections'][4].__setitem__('from', data['corrections'][4]['from'].replace('test -n', 'test'))),
    ]
    results = []
    with tempfile.TemporaryDirectory(prefix='r7-portable-audit-v5-selftest-') as temporary:
    ...
    require(len(results) == 8 and all(row['rejected'] for row in results), f'v5 self-tests failed: {results}')
    return {'schema': 'R7_PORTABLE_EXTERNAL_AUDIT_SELFTEST_V5', 'passed': True, 'caseCount': 8, 'passedCaseCount': 8, 'cases': results}
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix='r7-portable-audit-v5-') as temporary:
        base_dir = Path(temporary)
    ...
    report = {
        'schema': 'R7_PORTABLE_EXTERNAL_AUDIT_V5',
        'passed': True,
