...
def main() -> None:
    if len(sys.argv) != 5:
        fail('usage: execute-r7f-portable-v2-plan.py INPUT_YAML CORRECTED_YAML DIFF REPORT')
    ...
    bad = '../../../r7f-evidence'
    replacement = '$GITHUB_WORKSPACE/r7f-evidence'
    count = original.count(bad)
    if count < 1:
        fail('expected logging-path correction anchor is absent')
    corrected = original.replace(bad, replacement)
    if corrected.count(bad):
        fail('logging-path correction incomplete')
    ...
    diff = ''.join(difflib.unified_diff(
        ...
        fromfile=source_path.as_posix(), tofile=corrected_path.as_posix(),
    ))
    ...
    for index, step in enumerate(steps, start=1):
        ...
        executed.append(row)
        report = {
            'schema': 'R7F_CORRECTED_PLAN_EXECUTION_V1',
            'passed': all(item['exitCode'] == 0 for item in executed),
            'sourcePlan': str(source_path.relative_to(ROOT)),
            'correctedPlan': str(corrected_path.relative_to(ROOT)),
            'correctionCount': count,
            'correction': {'from': bad, 'to': replacement},
        ...
        if result.returncode != 0:
            fail(f'R7F plan step failed: {name} (exit {result.returncode})')
